	...
	<cfquery  dbtype="query" name="filters">
		...
		FROM 		programSchedule 
		WHERE 		LIVE 	= 'Live' 
		AND 		PROGRAM IN ('NFL Football','MLS Soccer','NHL Hockey','NBA Basketball','NASCAR Racing','College Football','College Basketball','College Baseball','MLB Baseball','PGA Tour Golf') 
		OR 			PROGRAM LIKE '%Bowling%' 
		OR 			PROGRAM = 'Tennis' 
		ORDER BY 	PROGRAM
	</cfquery>
