	<!--- EZCALENDAR CLASS --->
	<cfset  ezcalendar 	= CreateObject("component", "cfc.index")>

	<cfset  markets 	= CreateObject("component", "cfc.markets")>
	...
	<!--- MAPPING TIME ZONES  --->
	<cfset tzmapped 		=  ezcalendar.timezonemapping(zones.ABBREVIATION)>
	...
	<cfset sDate 			= DateFormat(DateAdd('d',-1,startDate), "yyyy-mm-dd")>
	...
	<cfset programming 			=  ezcalendar.search(sdate,endDate, gridSTime,gridETime, zoneid, '0')>	
	<cfset programSchedule 		=  ezcalendar.scheduleformater(programming)>
<cfdump var="#programSchedule#"   abort>
	...
	<!--- cfset wRange.sDate= DateAdd('d',1,rsDate) --->
