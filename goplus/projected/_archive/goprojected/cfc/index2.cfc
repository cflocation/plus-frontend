	...
	<cffunction name="search">
		...
		<!--- PROJECTED STARTDATE --->
		   ...
		   <cfset pjstartdate 	= dateFormat(broadcastcalendar(now(),dateAdd('d',7,now())).wdate,'yyyy-mm-ddT00:00:00Z')>
			...
			<cfhttp url="https://plusapi.showseeker.com/plus/projected" method="post">
			...
			<cfset programSchedule =queryNew("	airDate,		airDay,
											...
											"	CF_SQL_DATE,	CF_SQL_VARCHAR,
												CF_SQL_VARCHAR,	CF_SQL_DATE,	
												CF_SQL_VARCHAR,	CF_SQL_VARCHAR,
												CF_SQL_DATE,	CF_SQL_DATE,	
												CF_SQL_VARCHAR,	CF_SQL_VARCHAR,
												CF_SQL_VARCHAR,	CF_SQL_VARCHAR,
												CF_SQL_VARCHAR,	CF_SQL_VARCHAR,
												CF_SQL_DATE,	CF_SQL_DATE,
			...
			<cfloop  from="1" to="#ArrayLen(getGridByNetwork)#" index="i">
				...
				<cfif StructKeyExists(getGridByNetwork[i],"createdat")>
					<cfset QuerySetCell(programSchedule,"CREATEDAT",		'#getGridByNetwork[i].createdat#')>
				<cfelse>
					<cfset QuerySetCell(programSchedule,"CREATEDAT",		'')>
				</cfif>
				
				<cfif StructKeyExists(getGridByNetwork[i],"updatedat")>
					<cfset QuerySetCell(programSchedule,"UPDATEDAT",		'#getGridByNetwork[i].updatedat#')>
				<cfelse>
