<cfcomponent hint="ezgrids" output="yes">
	...
	<cffunction name="search">
		...
		<cfset searchObj 		= CreateObject("component", "cfc.SearchProjected")>
		...
		<!--- netwoks --->
		<cfset stations 		= ValueList(zone.NETWORKID)>

		<cfset timezoneabbr		= '#lcase(zone.ABBREVIATION)#'>

		
		<!--- build the string TMS DATA --->
		<cfset getGridByNetwork = searchObj.searchGracenote(timezoneabbr,stations,startdate, enddate, starttime, endtime, pjstartdate)>
		
		<cfdump var="#getGridByNetwork#" abort>


			
			
			<!--- build the string PROJECTED DATA --->			
			<cfset solrpr = 'http://solr.showseeker.com:8983/solr/select/?q=*%3A*&version=2.2&start=0&fq=projected:1&rows=3000&indent=on&wt=json&sort=tz_start_pst asc'>
			...
			<cfloop  from="1" to="#ArrayLen(getGridByNetwork)#" index="i">
				...
				<cfif getGridByNetwork[i].DURATION GT 6>
					...
					<cfset QuerySetCell(programSchedule,"startDate", 		"#Left(sDate,10)#")>
					<cfset QuerySetCell(programSchedule,"endDate", 			"#Left(eDate,10)#")>
