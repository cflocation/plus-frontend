<cfcomponent hint="ezgrids" output="yes">
	...
	<cffunction name="search">
		...
		<cfargument name="projected" default="0">		
			...
			<!--- cfset starttime 	= "#TimeFormat(gridstime,'HH:mm')#">
			<cfset endtime 		= "#TimeFormat(gridetime,'HH:mm')#" --->
		...
		<!--- PROJECTED STARTDATE --->
		   ...
		   <cfset pjstartdate 	= dateFormat(broadcastcalendar(now(),dateAdd('d',7,now())).wdate,'yyyy-mm-ddT00:00:00Z')>
			...
			<!--- netwok --->
			<cfset nets 		= "fq=">
			<cfset zone 		= stations(zoneid) >

			<cfloop query="zone">
				<cfset nets 		= nets&"stationnum:#NETWORKID#">
				<cfif NOT zone.IsLast()>
					<cfset nets 	= nets&"+">
				</cfif>	
			</cfloop>	
			...
			<cfset timezonestart 	= "tz_start_#lcase(zone.ABBREVIATION)#">
			<cfset timezoneend 		= "tz_end_#lcase(zone.ABBREVIATION)#">
			<cfset starttimetz 		= "start_#lcase(zone.ABBREVIATION)#">
			<cfset timezoneday 		= "day_#lcase(zone.ABBREVIATION)#">
			...
			<cfset solr = solr&"&fl=id,callsign,showtype,projected,packageid,showid,tmsid,live,genre,stars,descembed,orgairdate,title,premierefinale,new">
			...
			<!--- build the string PROJECTED DATA --->			
			<cfset solrpr = 'http://104.239.197.204:8983/solr/select/?q=*%3A*&version=2.2&start=0&fq=projected:1&rows=3000&indent=on&wt=json&sort=tz_start_pst asc'>
			<cfset solrpr = solrpr&"&fl=id,callsign,showtype,projected,packageid,showid,tmsid,live,genre,stars,descembed,orgairdate,title,premierefinale,new,epititle,#timezoneday#,#starttimetz#,#timezonestart#,#timezoneend#,duration,createdat,updatedat">
			<cfset solrpr = solrpr&"&#nets#">	
			<cfset solrpr = solrpr&"&#pjdaterange#">
			<cfset solrpr = solrpr&"&#timerange#">
			
			<cfhttp url="#solrpr#"><cfhttpparam type="CGI" encoded="false" name="Content_Type" value="application/json; charset=utf-8"></cfhttp>
			
			<cfset prData = DeserializeJSON(cfhttp.fileContent)>
			<cfset getGridByNetworkPR = prData.response.docs>			

			<cfset getGridByNetwork.addAll( getGridByNetworkPR )>

			<cfset programSchedule = queryNew("airDate,airDay,CALLSIGN,CREATEDAT,DAYNUM,DURATION,endDate,endTime,EPISODE,GENRE,LIVE,new,SHOWDESC,SKEDULEID,startDate,startTime,stationId,premiere,PREMIEREANDLIVE,PROGRAM,PROGRAM_KEY,tmsid,PROJECTED,PACKAGEID, UPDATEDAT")/>
