...
function filterZones(){
	...
	if($.inArray(parseInt(corpid),[14,46]) > -1){
		...
		.then(function(){
				if(roles.ezRatings === true){
					loadBooksByMarket(dmaId,myEzRating.getRatings('books'));
				}
			});		
	}
	else{
...
function ratingsZoneSynch(zoneid){
	
	if(!$.isEmptyObject(myEzRating)){
		if(!$.isEmptyObject(myEzRating.ratingsData)){
			for(var z=0; z<marketzones.length; z++ ){
				if(parseInt(marketzones[z].id) === parseInt(zoneid)){
					loadBooksByMarket(marketzones[z].dmaId);
					break;
				}
			}
		}
...
function zoneListBuilder(marketId, selectorList,dmaId){
	if((roles.ezRatings || ($.inArray(parseInt(corpid),[14,46]) > -1)) && dmaId !== undefined){
		$.each(marketzones, function(h, zone) {
