...
if(isset($_GET['proposalid'])){
	...
	/*
		...
		foreach($proposal->lines as $zone){
			$lines = array_merge($lines, $zone->lines);
		}

		usort($lines, function($a,$b){
			return strnatcmp ($a->sortingStartDate,$b->sortingStartDate);
			//return $a->sortingStartDate-$b->sortingStartDate;
		});
	*/
	...
	foreach($proposal->lines as $zone){
		foreach ($zone->lines as $line) {
			$codesObj  = mysql_fetch_object(mysql_query("SELECT gci.networkCode, IF((SELECT z.hd FROM ShowSeeker.zones as z WHERE z.id={$line->zoneId})>0,gci.networkCodeHD,'') AS networkCodeHD FROM Customers.GCIMapping as gci WHERE gci.tmsID = '{$line->stationId}'"));
	...
	//print("http://plus.showseeker.com/goplus/downloads/xmls/$name");
		...
		if(checkS3()){
			...
			print 'https://plus.showseeker.com/goplus/downloads/'.XML_FOLDER.$name;
		}
...
//https://plus.showseeker.com/goplus/downloads/eclipse_new.php?userid=176&key=75341a613bc1c5f4dabb47a6283d1a24&proposalid=96750&customer=0&salesperson=0&agency=0&ucBookend=N&ulLength=30&revenuetype=Agency
//https://plus.showseeker.com/goplus/downloads/eclipse_new.php?userid=176&key=75341a613bc1c5f4dabb47a6283d1a24&proposalid=97470&customer=0&salesperson=0&agency=0&ucBookend=N&ulLength=30&revenuetype=Agency
//http://plus.showseeker.com/goplus/downloads/xmls/eclipse_1.xml
