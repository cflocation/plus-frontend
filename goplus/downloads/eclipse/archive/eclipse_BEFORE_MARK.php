<?php 
	...
	if(isset($_GET['proposalid']))
	{
		...
		{
			$codesObj = mysql_fetch_object(mysql_query("SELECT networkCode, networkCodeHD FROM Customers.GCIMapping WHERE tmsID = '{$proposalLines[$i]->stationnum}'"));
			...
			{
				foreach($weeksSplit as $sWeek)
					$xmlOrderLines->appendChild(getOrderLineXml($xml, $lineNum++,$ucBookend,$proposalLines[$i]->linetype,$sWeek['startdate'],$sWeek['enddate'],$proposalLines[$i]->starttime,$proposalLines[$i]->endtime,$ulLength,$sWeek['spots'],$proposalLines[$i]->day,$codesObj->networkCodeHD, $proposalLines[$i]->zoneid, $proposalLines[$i]->rate));
			}
		}
...
function getOrderLineXml($xml, $lineNum, $ucBookend, $linetype, $startdate,$enddate,$starttime,$endtime, $ulLength,$spotsweek,$days,$networkCode,$zoneid,$rate)
{
	...
	else if($linetype=='Line')		$xmlQuantityType->appendChild($xml->createElement("ucQuantityType","L"));
...
function getWeeks($proposalLine)
{
	...
	for($i=0;$i<$proposalLine->weeks;$i++)
	{
		...
		$weeksArr[] = array(
						'startdate'=> ($itStartMonday<$proposalLine->startdate)?$proposalLine->startdate:date("Y/m/d",$itStartMonday)
						,'enddate'=> ($itEndSunday<$proposalLine->enddate)?$proposalLine->enddate:date("Y/m/d",$itEndSunday)
						,'propName'=> $propName
