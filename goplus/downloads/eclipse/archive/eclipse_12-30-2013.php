...
function getWeeks($proposalLine)
{
	...
	for($i=0;$i<$proposalLine->weeks;$i++)
	{
		...
		$weeksArr[] = array(
						'startdate'=> ($itStartMonday<strtotime($proposalLine->startdate))?$proposalLine->startdate:date("Y/m/d",$itStartMonday)
						,'enddate'=> ($itEndSunday<strtotime($proposalLine->enddate))?$proposalLine->enddate:date("Y/m/d",$itEndSunday)
						,'propName'=> $propName
