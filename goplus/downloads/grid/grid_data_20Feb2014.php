...
<body>
      ...
      <ul id="tabs">
          ...
          <?php if(isset($arrWeekMondays) && !empty($arrWeekMondays)){
              foreach($arrWeekMondays as $mondayKey => $mondays){
                  ...
                  echo '<li><a href="#" name="#tab'.($mondayKey+1).'">'.$month.' '.date('d',strtotime($mondays)).'</a></li>';
              }
          } ?>
    ...
    <div id="content">  <!-- Printing tabs contents -->
      <?php if(isset($arrWeekMondays) && !empty($arrWeekMondays)){ ?>
        <?php foreach($arrWeekMondays as $mondayKey => $mondays){ ?>
                ...
                <table class="table-area" cellspacing="0" cellpadding="0">
                   ...
                   <!-- Calculation for printing actual data -->
                    ...
                    <?php for($i=0;$i<36;$i++){ ?>                         
                        ...
                        <tr>
                            <td style="font-size:11px;border-color: #cccccc;" class="td_time"><?php echo $RowTime; ?></td> <!-- Left side time -->
                            <?php foreach($arrWeeklyDays as $weekKey => $weekVal){ ?>
                                <?php if(isset($arrStation[$weekVal]) && !empty($arrStation[$weekVal])){ ?>
                                        <?php if(isset($arrStation[$weekVal][$RowTime]) && !empty($arrStation[$weekVal][$RowTime])){ ?> <!-- If this week day contain this time data -->
                                                    ...
                                                    ?>
                                                    <td valign="top" style="font-size:11px;word-wrap:break-word;" rowspan="<?php echo $arrStation[$weekVal][$RowTime]['row']; ?>"><input type="checkbox" /><?php echo $arrStation[$weekVal][$RowTime]['title']; ?></td>
                                ...
                                <?php } ?>
                            <?php } ?>
                            <td style="font-size:11px;border-color: #cccccc;" class="td_time"><?php echo $RowTime; ?></td> <!-- Right side time -->
                        </tr>
                        <?php } ?>
                </table>
            </div>
