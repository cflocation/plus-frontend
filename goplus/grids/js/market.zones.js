...
function setDmas(){
	...
	if($.inArray(parseInt(sswin.corpid),[14,46]) === -1){
	...
	if($.inArray(parseInt(sswin.corpid),[14,46]) >= 0){
		$('#usrdmas').parent('.row').show();
	}
/*
	if('ezRatings' in sswin.roles || $.inArray(parseInt(sswin.corpid),[14,46]) >= 0){
...
function setZones(){
	...
	if(sswin.roles.ezRatings || $.inArray(parseInt(sswin.corpid),[14,46]) >= 0){
		for(key in ssZones){
