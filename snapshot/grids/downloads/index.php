<?php
	...
	$zid			= $_GET['zid'];
	$timeZone		= $_GET['tz'];	
	$netCallsign	= $_GET['callsign'];
	$filename 		= $netCallsign.'.pdf';
...
function getSchedFromSolr($fromDate,$toDate,$station,$startTimeG,$endTimeG,$zone){
	...
	while(strtotime($toDate)>=strtotime($iDate)){		
		$solrUrl  = "http://10.15.10.102:8983/solr/snapshot/select/?q=*%3A*&version=2.2&start=0&indent=on&wt=json&rows=5000&fq=tz_start_{$zone}:[{$iDate}T00:00:00Z%20TO%20{$iDate}";
		...
		$currentDate=date('Y-m-d',strtotime($fromDate."+$i days")); 
			...
			foreach($schedule[$i] AS $sh){
				...
				if($sh['duration']>=7){
					...
					if(isset($tmpProgram['lastProgram']) && $tmpProgram['lastProgram']!= ""){
							...
							foreach($schedule[0] as $val){
								...
								if($val['timesincemid']==0){
									if(strtotime($startLastVal)==strtotime($fromDate)){
										...
										$xVal = 22;
										$pdf->writeHTMLCell(25, $tmpProgram['lastProgram']['height'],$xVal,15, $tmpProgram['lastProgram']['html'], 1, 0, 0, true, 'C', true);
										$temp=1;
										break;
									}
								}
				...
				if(isset($lastProgramArr))
					$tmpProgram['lastProgram']=$lastProgramArr;
				}
			}
		$x = 22+(25*$i);
		$y = 10+(($n+1)*$height);
