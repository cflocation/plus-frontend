...
print_r('<pre/>');	
	...
	$netInfo	= new Station($con);
	$net		= $netInfo->getInfo($station, $zid);	
		
	$filename 	= $net['callsign'].'.pdf';
							
	$bucket 	= strtolower(getcwd()."/temp/$filename");
	$tz 		= strtolower($net['abbreviation']);	
	$zone 		= strtolower($net['abbreviation']);
	...
	// set option to download current page view (1) / All weeks view(2)
	$option=1; 
	
	//set the option based on the above value (we can reamove this switch later )
	if($option==1){
		$v=1;
	}
	elseif($option==2){
...
function getSchedFromSolr($fromDate,$toDate,$station,$startTimeG,$endTimeG,$zone){
	...
	while(strtotime($toDate)>=strtotime($iDate)){		
		$solrUrl  = "http://10.15.10.102:8983/solr/gracenote/select/?q=*%3A*&version=2.2&start=0&indent=on&wt=json&fq=projected:0&rows=5000&fq=tz_start_{$zone}:[{$iDate}T00:00:00Z%20TO%20{$iDate}";
		...
		if(strtotime($currentDate)>=strtotime($dateToday)){
			...
			foreach($schedule[$i] AS $sh){
				...
				if($sh['duration']>=7){
					...
					if($sh['heightfactor']== 1)
						$html='<div style="overflow: hidden; max-height:' .$height*$sh['heightfactor']. 'mm;"><span style="display:inline-block; vertical-align:middle; color:'.$color.'">'.$title.$live.$epititle.'</span></div>';
					if($sh['heightfactor']== 2)
						$html='<div style="overflow: hidden; max-height:' .$height*$sh['heightfactor']. 'mm;"><span style="display:inline-block; vertical-align:middle; color:'.$color.'"><BR>'.$title.$live.$epititle.'</span></div>';
					if($sh['heightfactor']== 3)
						$html='<div style="overflow: hidden; max-height:' .$height*$sh['heightfactor']. 'mm;"><span style="display:inline-block; vertical-align:middle; color:'.$color.'"><BR><BR>'.$title.$live.$epititle.'</span></div>';	
					if($sh['heightfactor']== 4)
						$html='<div style="overflow: hidden; max-height:' .$height*$sh['heightfactor']. 'mm;"><span style="display:inline-block; vertical-align:middle; color:'.$color.'"><BR><BR><BR>'.$title.$live.$epititle.'</span></div>';	
					if($sh['heightfactor'] > 4)
						$html='<div style="overflow: hidden; max-height:' .$height*$sh['heightfactor']. 'mm;"><span style="display:inline-block; vertical-align:middle; color:'.$color.'"><BR><BR><BR><BR>'.$title.$live.$epititle.'</span></div>';							
