...
$(document).ready(function() {
    ...
    if (localStorage.getItem("userId") === null || localStorage.getItem("apiKey") === null) {
        window.location.href = "../login.php?logout=true&app=rcm";
    } else {
        $.ajax({
            type:'post',
...
function getRCGroups(){
	var url = "services/groups.php?eventtype=list&marketid="+marketsid;
	
	$.getJSON(url, function(data){

    $('#rc-name').find('option').remove().end().append('<option value="0">Group Name</option>');
		
		$.each(data,function(i,value){
			$.each(value,function(j,val){
			$('#rc-name').append($('<option></option>').attr('value', val.name).text(val.name));
			});
		});
	});
}
...
function loadRatecardByID(id, type) {
    ...
    $.getJSON("services/ratecards.php?eventtype=loadratecard&id=" + id + "&type=" + type, function(data) {
        ...
        if(parseInt(marketsid) === 66){
	        var tmpDayParts = [];
	        var oct282016, currentDate;
	        
	        oct272016 =  new Date.parse('2016/10/27');
	        
	        $.each(data.dayparts, function(i,val){
		        currentDate = new Date.parse(val['createdat']);
		        if(currentDate > oct272016){
			        tmpDayParts.push(val);
		        }
	        });
	        dayparts = tmpDayParts;
		}
		else{
        ...
        if (info.broadcast == 1) {
            ...
            $('#btn-hot-copy-lines').css('display', 'none');
            $('#btn-hot-copy-all').css('display', 'none');
            $('#message-hot-1').css('display', 'none');
            ...
            $('#btn-hot-copy-lines').css('display', 'inline');
            $('#btn-hot-copy-all').css('display', 'inline');
            $('#message-hot-1').css('display', 'inline');
        }
...
function saveRatecardChangesNoDialog() {
...
function saveRatecardChanges(evt) {
    ...
    $.post("services/ratecards.php", {
        ...
        if (evt == "publish-group") {
            loadDialogWindow('publish-ratecard-group', 'Publish Selected Ratecards', 380, 200);
        }
...
function publishRatecardGroupEvent() {
    var ratecardid = datagridRatecards.selectedIds();

    $.post("services/ratecards.php", {
        eventtype: "publish-group",
        ratecardid: ratecardid
    }).done(function(data) {
        closeAllDialogs();
        getCardsForMarket(selectedmarketid, false);
...
$('#form-new-ratecard')
    ...
    .on('valid', function() {
        ...
        if($('#ratecard-name').is(':visible')){
	        var name = $('#ratecard-name').val();	        
        }
        else{
	        var name = $('#rc-name').val();
	        if(name == 0){
		        alert('Select or add a Rate Card Name');
		        return;
	        }
...
function saveHotProgrammingSilent() {
    if (zoneid == 0) {
        return;
    }
    var rows = datagridHotProgramming.getData();
...
$("#pricing-title-selector-broadcast").change(function() {
    $('#pricing-title-alt-selector-broadcast').val("");
});

// wire up the search textbox to apply the filter to the model
$("#pricing-filter-broadcast").keyup(function(e) {
    datagridPricingBroadcast.updatFromKeyword(e, this.value);
});
...
function addBroadcastRate() {
    ...
    var weekdays = $('#ratecard-broadcast-days').val();
    var daysid = weekdays.join(",").replace(/,/g, "");
    var weekdays2 = weekdays.join(",");


    var starttime = $('#ratecard-broadcast-start-time').val();
    var endtime = $('#ratecard-broadcast-end-time').val();

    var starttimeID = Date.parse(starttime).toString("HHmmss");
    var endtimeID = Date.parse(endtime).toString("HHmmss");

    var starttimeFormat = Date.parse(starttime).toString("HH:mm:ss");
    var endtimeFormat = Date.parse(endtime).toString("HH:mm:ss");
    ...
    row.fname = fname;
    row.starts = starttimeFormat;
    row.stops = endtimeFormat;
    row.weekdays = weekdays2;
...
function stripSearch(title){
    var a = title.replace(/[^a-z0-9]/gi,'');
    var b = a.toLowerCase();
    return b;
}
...
function searchBroadcastTitles(title) {
    var selectrow = 0;
    ...
    $.post("services/ratecards.php", {
        ...
        if (json.titles.length == 0) {
            $('#pricing-title-selector-broadcast').append($("<option></option>").attr("value", 0).text("No Titles Available"));
        } else {
            $('#pricing-title-selector-broadcast').append($("<option></option>").attr("value", 0).text("Select Title"));
            $.each(json.titles, function(i, value) {
                if(stripSearch(value) == title){
                    selectrow = value;
                }
        ...
        if(selectrow != 0){
            //$('#pricing-title-selector-broadcast select').val(selectrow);
            $('#pricing-title-selector-broadcast [value="'+selectrow+'"]').prop('selected', true);
            $('#pricing-title-alt-selector-broadcast').val('');
        }
    });
}
...
function resetSidebar(){
    $('#pricing-filter-broadcast').val('');
    datagridPricingBroadcast.updatFromKeyword(0, '');


    $('#pricing-title-alt-selector-broadcast').val('');
    $('#ratecard-broadcast-days').val('1,2,3,4,5,6,7');

    $('#ratecard-broadcast-start-time').val('06:00 AM');
    $('#ratecard-broadcast-end-time').val('11:59 PM');

    $('#pricing-daypart-broadcast').val('');
    $('#pricing-fixed-broadcast').val('');
...
function copySelectedRatecards() {
    ...
    if (cnt.length == 0 || cnt.length > 1) {
	  	loadDialogWindow('copy-multiple-ratecards', 'Copy Selected Ratecards', 350, 440);
    }
    else{
...
function toggleCols(i){
	setTimeout(function(){
		if($(i).hasClass('expanded')){
			$('#collapse-global').removeClass('expanded').addClass('collapsed');
		}		
	}, 100);
...
function toggleGlobalCollapse(){
	if($('#collapse-global').hasClass( "expanded" )){
		$('#collapse-global').removeClass('expanded').addClass('collapsed');
		datagridRatecards.collapseAllGroups();
	}
	else{
		$('#collapse-global').removeClass('collapsed').addClass('expanded');
		datagridRatecards.groupByColumn('name');
	}
}
