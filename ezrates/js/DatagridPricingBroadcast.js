...
function DatagridPricingBroadcast() {
    ...
    function myFilter(item, args) {
        ...
        if(args.searchString !== "" && item.fname.toLowerCase().indexOf(args.searchString.toLowerCase()) === -1) {
            return false;
        }
        ...
        dataview.sort(function(row1, row2) {
            ...
            for (var i = 0, l = sortCols.length; i < l; i++) {
                ...
                if(field == "weekdays")
                {
                    ...
                    tx[0] = (tx.length == 7) ? 2 : tx[0];
                    ty[0] = (ty.length == 7) ? 2 : ty[0];
                    
                    tx[0] = (x == '1,7') ? 7 : tx[0];
                    ty[0] = (y == '1,7') ? 7 : ty[0];

                    x= (tx[0] == '1') ? 7: parseInt(tx[0])-1;
                    y= (ty[0] == '1') ? 7: parseInt(ty[0])-1;
                }
    ...
    this.initSort = function(){
        ...
        gridSorter([sort2,sort1], dataView);
    }
    ...
    this.parseBroadcastLine = function(row) {
        if (editmode) {
            ...
            var a = row.fname.replace(/[^a-z0-9]/gi,'');
            //a = a.replace(/[^a-z0-9]/gi,'');
            var b = a.toLowerCase();


            searchBroadcastTitles(b);
        }
    }
    ...
    this.addRow = function(row) {
        ...
        if (f == false) {
            row['sort_title'] = row['fname'].toLowerCase();
            data.push(row);
        }else{
