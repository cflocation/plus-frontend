<?php
	...
	function allRatecardsForZone($zoneid,$group,$startdate){
		...
		FROM ratecard_cards
		INNER JOIN ratecard_card_versions ON ratecard_card_versions.ratecardid = ratecard_cards.id
		WHERE zoneid = $zoneid  AND ratecard_cards.deletedat IS NULL AND ratecard_card_versions.published = 1  AND ratecard_cards.startdate <= '$startdate' AND ratecard_cards.enddate >= '$startdate'
