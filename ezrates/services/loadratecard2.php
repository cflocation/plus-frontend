<?php
	...
	if($cardid == 0){
		...
		if($cnt == 0){
			...
			INNER JOIN zones ON zones.id = ratecard_cards.zoneid
			INNER JOIN ratecard_card_versions ON ratecard_card_versions.ratecardid = ratecard_cards.id
			WHERE ratecard_cards.zoneid = $zoneid  AND ratecard_card_versions.published = 1 AND ratecard_cards.deletedat IS NULL ORDER BY ratecard_cards.id DESC LIMIT 1";
			$result = mysql_query($sql);
		}	
	...
	function buildHeader($row){
		...
		$header = array("location"=>"www.showseeker.com/ezrates/services/loadratecard2.php", "ratecardtype"=>4, "startdate"=>$longstart, "enddate"=>$longend,"zoneid"=>$zoneid,"zone"=>$zone,"name"=>$row['name'], "label"=>$label, "id"=>$id);
		return $header;
	}
	...
	function allRatecardsForZone($zoneid,$group,$startdate){
		...
		FROM ratecard_cards
		INNER JOIN ratecard_card_versions ON ratecard_card_versions.ratecardid = ratecard_cards.id
		WHERE zoneid = $zoneid  AND ratecard_cards.deletedat IS NULL AND ratecard_card_versions.published = 1 AND ratecard_cards.enddate >= '$todayis'
