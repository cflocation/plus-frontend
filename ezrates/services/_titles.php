<?php
	...
	if($event == "title"){
		...
		/*print_r('http://162.242.168.97:8983/solr/gracenote/select?q=*%3A*&fq='.urlencode($q).'&fl=title&wt=json&indent=true&group=true&group.field=title&rows=1000');
		exit;


		//http://162.242.168.97:8983/solr/gracenote/select?q=*%3A*&fq=title%3Anba&fl=title&wt=json&indent=true&group=true&group.field=showid
		$resultOld = file_get_contents('http://162.242.168.97:8983/solr/gracenote/select?q=*%3A*&fq='.urlencode($q).'&fl=title&wt=json&indent=true&group=true&group.field=title&rows=1000');
		$resultOld = json_decode($resultOld);


		foreach ($resultOld->grouped->title->groups as &$value) {
		...
		//$resultNew = file_get_contents('http://solr.showseeker.net:8983/solr/gracenote/select?q=*%3A*&fq='.urlencode($q).'&fl=title&wt=json&indent=true&group=true&group.field=title&rows=1000');
		$resultNew = file_get_contents('http://solr.showseeker.net:8983/solr/gracenote/select?&q=*:*&fq='.urlencode('-genre1:"adults only"').'&fq='.urlencode('-genre2:"adults only"').'&fq='.urlencode($q).'&group=true&group.field=title&fl=title&rows=1000');
