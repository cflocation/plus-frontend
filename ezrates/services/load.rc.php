<?php
	...
	function buildHeader($row){
		...
		$header 		= array(	"location"=>"www.showseeker.com/ezrates/services/load.rc.php", 
									"ratecardtype"=>4, 
									"startdate"=>$longstart, 
									"enddate"=>$longend,
									"zoneid"=>$zoneid,
	...
	function getRates($syscode, $eDate){
		...
		$sql_rate = "SELECT 	Customers.charter_ratecards.rate as rate,
									...
									Customers.charter_ratecards.start_time as starts,
									Customers.charter_ratecards.end_time as stops,
									Customers.charter_ratecards.days as weekdays,
									Customers.charter_ratecards.daypart as id,
									Customers.charter_ratecards.network_id as networkid,
									'' as fname,
									ShowSeeker.tms_networks.callsign
		FROM 						Customers.charter_ratecards
		LEFT OUTER JOIN		ShowSeeker.tms_networks
		ON							Customers.charter_ratecards.NETWORK_ID =	ShowSeeker.tms_networks.networkid
		...
		while($station = mysql_fetch_assoc($result)){
			...
			$netrates 	= queryOfQuery($result_rates, "*", false, "networkid", $networkid);
			$network 	= array();
			
			foreach($netrates as &$rates){
				array_push($network, $rates);
			}
