<?php
	...
	function buildHeader($row){
		...
		$header = array("location"=>"www.showseeker.com", "ratecardtype"=>4, "startdate"=>$longstart, "enddate"=>$longend,"zoneid"=>$zoneid,"zone"=>$zone,"name"=>$row['name'], "label"=>$label, "id"=>$id);
		return $header;
	}
	...
	function getRates($ratecardid, $dayparts){
		...
		WHERE  ratecard_card_versions.ratecardid = $ratecardid AND ratecard_card_versions.working = 1
		LIMIT 1";
		$result = mysql_query($sql);
		$row = mysql_fetch_assoc($result);
		$rates = $row['ratecard'];
