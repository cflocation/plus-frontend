<?php
	...
	if(isset($_COOKIE["tokenid"]) && isset($_COOKIE["userid"])){
		...
		$_SESSION['tokenid'] = $_COOKIE["tokenid"];
	...
	if(isset($_POST['email'])){
		...
		$sql = "SELECT * FROM users WHERE email = '{$email}' AND password = '{$password}' AND deletedat IS NULL LIMIT 1";
		...
		if($num_rows == 1){
			...
			$userid = $_SESSION['userid'];
			header( 'Location: process_zips.php?uid='.$userid ) ;
		}else{
			$error = true;
		}
...
<form class="formee" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
